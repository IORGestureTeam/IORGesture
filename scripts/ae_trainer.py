    ...
    def train(self, epoch):
        ...
        for its, dict_data in enumerate(self.train_loader):
            ...
            poses_feat, pose_mu, pose_logvar, recon_data = \
                self.model(tar_pose) #variational_encoding=self.variational_encoding
        ...
        with torch.no_grad():
            ...
            for its, dict_data in enumerate(self.test_loader):
                ...
                if "LSTM" in self.g_name or "multi_length" in self.notes:
                    poses_feat, pose_mu, pose_logvar, recon_data = \
                    self.model(**dict(inputs=tar_pose,))
                    out_final = (recon_data.cpu().numpy().reshape(-1, self.train_pose_dims) * self.std_pose) + self.mean_pose                  
                else:
                    for i in range(tar_pose.shape[1]//(self.pose_length)):
                        ...
                        poses_feat, pose_mu, pose_logvar, recon_data = \
                    self.model(**dict(inputs=tar_pose_new,))
                        out_sub = (recon_data.cpu().numpy().reshape(-1, self.train_pose_dims) * self.std_pose) + self.mean_pose
                        if i != 0:
