...
import pickle
import numpy as np
from utils.other_tools import ignore_none_cat
...
class TextEncoderTCN(nn.Module):
    ...
    def __init__(self, args, n_words, embed_size=300, pre_trained_embedding=None,
        ...
        self.decoder = nn.Linear(num_channels[-1], 128) #args.word_f)
...
class WavEncoder(nn.Module):
    def __init__(self, out_dim):
        ...
        self.feat_extractor = nn.Sequential( 
                BasicBlock(1, 32, 15, 5, first_dilation=1066, downsample=True), #(original padding:1600)
...
class RealtimeLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, batch_first, bidirectional, dropout):
        ...
        self.hidden_num_layers = num_layers
        if bidirectional:
            self.hidden_num_layers *= 2
        self.hidden_size = hidden_size
        self.LSTM = nn.LSTM(input_size, hidden_size=self.hidden_size, num_layers=num_layers, batch_first=batch_first,
            bidirectional=bidirectional, dropout=dropout)
    def flatten_parameters(self):
        self.LSTM.flatten_parameters()
    def forward(self, x, h, c):
        #out, (h_new, c_new) = self.LSTM(x) #debug
        out, (h_new, c_new) = self.LSTM(x, (h, c))
        return out[:, -1, :], (h_new, c_new)
...
class PoseGenerator(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.in_size = self.audio_f + self.train_pose_dims + self.facial_f + 1
        ...
        # learn head rotation
        self.head_rot_f = args.head_rot_f #12
        self.head_rot_hidden = 36
        self.head_rot_encoder = nn.Sequential(
            nn.Linear(6, self.head_rot_hidden), #rot-6d
            nn.ReLU(),
            nn.Linear(self.head_rot_hidden, self.head_rot_f), 
        )
        self.in_size += self.head_rot_f
        ...
        self.LSTM = RealtimeLSTM(self.in_size, hidden_size=self.hidden_size, num_layers=args.n_layer, batch_first=True,
                          bidirectional=False, dropout=args.dropout_prob)
        self.out = nn.Sequential(
            nn.Linear(self.hidden_size, self.hidden_size//2),
            nn.LeakyReLU(0.1, True),
            nn.Linear(self.hidden_size//2, 9*self.train_joint_dims)
        )
        
        #self.LSTM_hands = nn.LSTM(self.in_size+27, hidden_size=self.hidden_size, num_layers=args.n_layer, batch_first=True,
        #                  bidirectional=True, dropout=args.dropout_prob)
        self.LSTM_hands = RealtimeLSTM(self.in_size, hidden_size=self.hidden_size, num_layers=args.n_layer, batch_first=True,
                          bidirectional=False, dropout=args.dropout_prob)
        self.out_hands = nn.Sequential(
            nn.Linear(self.hidden_size, self.hidden_size//2),
            nn.LeakyReLU(0.1, True),
            nn.Linear(self.hidden_size//2, (47-9)*self.train_joint_dims)
        )
    ...
    def forward(self, pre_seq, in_audio=None, in_facial=None, in_text=None, in_id=None, in_emo=None, is_test=False):
        ...
        # not reachable code...? (test, train camn)
...
class CaMN(PoseGenerator):
    def __init__(self, args):
        ...
        if args.word_rep is not None:
            self.audio_fusion_dim += self.word_f
        self.facial_fusion_dim = self.audio_fusion_dim + self.facial_f# + self.head_rot_f
    ...
    def align_feat_seq_length(self, feat, seq_length):
        gap = seq_length - feat.shape[1]
        if gap > 0:
            feat = torch.cat((feat, feat[:, -gap:, :]), 1)
        elif gap < 0:
            feat = feat[:, :seq_length, :]
        return feat
    def forward(self, pre_seq, in_audio=None, in_facial=None, in_text=None, in_id=None, in_emo=None, in_head=None, lstm_hidden=None, lstm_hands_hidden=None):
        if self.do_flatten_parameters:
            self.LSTM.flatten_parameters()
        ...
        if self.speaker_embedding: 
            ...
            speaker_feat_seq = speaker_feat_seq.repeat(1, seq_length, 1)
            in_data =  ignore_none_cat((in_data, speaker_feat_seq), 2)

        if self.emotion_embedding:
            emo_feat_seq = self.emotion_embedding(in_emo).permute([0,2,1])
            emo_feat_seq = self.emotion_embedding_tail(emo_feat_seq).permute([0,2,1])
            in_data = ignore_none_cat((in_data, emo_feat_seq), 2)
            
        if self.word_f is not 0:
            text_feat_seq, _ = self.text_encoder(in_text)
            in_data = ignore_none_cat((in_data, text_feat_seq), 2)

        # Now, in_data includes: speakerID, emotion, text

        if self.audio_f is not 0:
            audio_feat_seq = self.audio_encoder(in_audio)
            audio_feat_seq = self.align_feat_seq_length(audio_feat_seq, seq_length)
            cat = ignore_none_cat((audio_feat_seq, in_data), dim=2)
            audio_fusion_seq = self.audio_fusion(cat.reshape(-1, self.audio_fusion_dim))
            audio_feat_seq = audio_fusion_seq.reshape(*audio_feat_seq.shape)
            in_data = ignore_none_cat((in_data, audio_feat_seq), 2)
        
        # Now, in_data includes: speakerID, emotion, text, audio


        if self.facial_f is not 0:
            face_feat_seq = self.facial_encoder(in_facial.permute([0, 2, 1])).permute([0, 2, 1])
            face_feat_seq = self.align_feat_seq_length(face_feat_seq, seq_length)
            cat = ignore_none_cat((face_feat_seq, in_data), dim=2)
            face_fusion_seq = self.facial_fusion(cat.reshape(-1, self.facial_fusion_dim))
            face_feat_seq = face_fusion_seq.reshape(*face_feat_seq.shape)
            in_data = ignore_none_cat((in_data, face_feat_seq), 2)
        
        # Head Rotation is considered as a base feature like word/emo/sem

        if self.head_rot_f is not 0:
            in_data = ignore_none_cat((in_data, self.head_rot_encoder(in_head)), 2)

        in_data = ignore_none_cat((pre_seq, in_data), dim=2)


        #print(f"<CaMN>start pose hands prediction ")

        output, lstm_hidden = self.LSTM(in_data, *lstm_hidden)
        #print(f"<CaMN>pose output done ")
        output = self.out(output)
        #print(f"<CaMN>pose output shape {output.shape} ")
        # input latest frame body pose
        
        in_data_hands = in_data.clone()
        in_data_hands[:, -1, 0:6*self.train_joint_dims] = output[:, 0:6*self.train_joint_dims]
        in_data_hands[:, -1, 25*self.train_joint_dims:28*self.train_joint_dims] = output[:, 6*self.train_joint_dims:9*self.train_joint_dims]
        output_hands, lstm_hands_hidden = self.LSTM_hands(in_data_hands, *lstm_hands_hidden)
        #print(f"<CaMN>hands output done ")
        output_hands = self.out_hands(output_hands)
        #print(f"<CaMN>hands output shape {output_hands.shape}")
        # input latest frame hands pose
        decoder_outputs_final = torch.zeros((batch_size, 47*self.train_joint_dims)).cuda()
        decoder_outputs_final[:, 0:6*self.train_joint_dims] = output[:, 0:6*self.train_joint_dims]
        decoder_outputs_final[:, 6*self.train_joint_dims:25*self.train_joint_dims] = output_hands[:, 0:19*self.train_joint_dims]
        decoder_outputs_final[:, 25*self.train_joint_dims:28*self.train_joint_dims] = output[:, 6*self.train_joint_dims:9*self.train_joint_dims]
        decoder_outputs_final[:, 28*self.train_joint_dims:47*self.train_joint_dims] = output_hands[:, 19*self.train_joint_dims:47*self.train_joint_dims]
        #print(f"<IORGesture>decoder_outputs_final shape: {decoder_outputs_final.shape}") # (256, 141)
        return decoder_outputs_final, lstm_hidden, lstm_hands_hidden

    
class ConvDiscriminator(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.input_size = args.joint_nums * args.train_joint_dims
        ...
        self.out2 = nn.Linear(args.pose_length-args.pre_frames-6, 1)
