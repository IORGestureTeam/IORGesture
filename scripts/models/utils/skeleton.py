...
class SkeletonResidual(nn.Module):
    def __init__(self, topology, neighbour_list, joint_num, in_channels, out_channels, kernel_size, stride, padding, padding_mode, bias, extra_conv, pooling_mode, activation, last_pool):
        ...
        seq.append(nn.GroupNorm(6, out_channels))  # FIXME: REMEMBER TO CHANGE BACK !!! # it was 10
    ...
    def forward(self, input):
        #print(f"SkeletonResidual input shape {input.shape}")
        #print(f"SkeletonResidual residual {self.residual(input).shape} shortcut {self.shortcut(input).shape}")
