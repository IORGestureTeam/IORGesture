...
class PoseGenerator(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.pose_dims = args.joint_nums * args.train_joint_dims
        ...
        self.LSTM_hands = nn.LSTM(self.in_size+9*self.train_joint_dims, hidden_size=self.hidden_size, num_layers=args.n_layer, batch_first=True,
                          bidirectional=True, dropout=args.dropout_prob)
...
class CaMN(PoseGenerator):
    ...
    def align_feat_seq_length(self, feat, seq_length):
        ...
        return feat
    def forward(self, pre_seq, in_audio=None, in_facial=None, in_text=None, in_id=None, in_emo=None, in_head=None):
        if self.do_flatten_parameters:
            self.LSTM.flatten_parameters()
        ...
        if self.head_rot_f is not 0:
            in_data = torch.cat((in_data, self.head_rot_encoder(in_head)), 2)
        ...
        decoder_outputs_final = torch.zeros((in_data.shape[0], in_data.shape[1], 47*self.train_joint_dims)).cuda()
        decoder_outputs_final[:, :, 0:6*self.train_joint_dims] = decoder_outputs[:, :, 0:6*self.train_joint_dims]
        decoder_outputs_final[:, :, 6*self.train_joint_dims:25*self.train_joint_dims] = decoder_outputs_hands[:, :, 0:19*self.train_joint_dims]
        decoder_outputs_final[:, :, 25*self.train_joint_dims:28*self.train_joint_dims] = decoder_outputs[:, :, 6*self.train_joint_dims:9*self.train_joint_dims]
        decoder_outputs_final[:, :, 28*self.train_joint_dims:47*self.train_joint_dims] = decoder_outputs_hands[:, :, 19*self.train_joint_dims:47*self.train_joint_dims]
...
class ConvDiscriminator(nn.Module):
    def __init__(self, args):
        ...
        self.out2 = nn.Linear(args.pose_length-6, 1)
