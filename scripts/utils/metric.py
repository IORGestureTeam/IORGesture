...
class SRGR(object):
    def __init__(self, threshold=0.1, joints=47, joint_data_dims=6):
        self.threshold = threshold
        self.joints = joints
        self.joint_data_dims = joint_data_dims
        self.counter = 0
        self.sum = 0
        
    def run(self, results, targets, semantic):
        results = results.reshape(-1, self.joints, self.joint_data_dims)
        targets = targets.reshape(-1, self.joints, self.joint_data_dims)
        ...
        for i in range(success.shape[0]):
            # srgr == 0.165 when all success, scale range to [0, 1] # cannot understand this
            success[i, :] *= semantic[i] # * (1/0.165)  # max(semantic) = ? i think it is 1, not 0.165
...
class alignment(object):
    def __init__(self, args, sigma, order):
        self.joint_dims = args.eval_joint_dims
        self.sigma = sigma
        self.order = order
