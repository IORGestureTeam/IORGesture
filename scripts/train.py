...
class BaseTrainer(object):
    def __init__(self, args):
        ...
        self.output_pose_dims = args.joint_nums * args.output_joint_dims
        ...
        self.l2_loss = get_loss_func("l2_loss")
        # TODO: 
        # self.pos_loss        
        ...
        self.test_loader = torch.utils.data.DataLoader(
            self.test_data, #beat.py CustomDataSet
            batch_size=1,  
            shuffle=False,  
            num_workers=args.loader_workers,
            drop_last=False,
        ...
        model_module = __import__(f"models.{args.model}", fromlist=["something"]) # models.camn.py
        
        if self.ddp:
            self.model = getattr(model_module, args.g_name)(args).to(self.rank) # CaMN
        ...
        '''
        self.metric_calculator = other_tools.Metric_calculator(self.args, self.rank)
        
        self.opt = create_optimizer(args, self.model)
        self.opt_s = create_scheduler(args, self.opt)
    def normalize_pose_gpu(self, pose):
        return (pose - self.mean_pose_gpu) / self.std_pose_gpu
